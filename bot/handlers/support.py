...
class SupportHandler:
    ...
    async def reply_to_ticket(self, callback: types.CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            "💬 **Ответ на обращение**\n\n"
            f"📋 Тикет #{ticket_id}\n\n"
            "✍️ Напишите ваш ответ или дополнительную информацию по обращению.\n\n"
            "💡 Ваш ответ будет добавлен к существующему тикету.",
            parse_mode="Markdown"
        )
        
        # Переводим в режим добавления ответа к тикету  
        await state.set_state(SupportStates.entering_message)
        await state.update_data(
            reply_to_ticket_id=ticket_id,
            category="reply"
        )
